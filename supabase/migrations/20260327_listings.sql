...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_listing_owner'
          AND conrelid = 'public.listings'::regclass
    ) THEN
        ALTER TABLE public.listings
...
CREATE UNIQUE INDEX IF NOT EXISTS one_review_per_user ON public.reviews(listing_id, user_id);

CREATE INDEX IF NOT EXISTS idx_listings_address ON public.listings(address);
...
DROP TRIGGER IF EXISTS update_listing_rating_after_insert ON public.reviews;
...
DROP TRIGGER IF EXISTS update_listing_rating_after_delete ON public.reviews;
...
DROP TRIGGER IF EXISTS update_listing_rating_after_update ON public.reviews;
