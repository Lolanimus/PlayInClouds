...
begin
  ...
  select r.renter_id, l.owner_id, r.listing_id, l.title, r.start_at, r.end_at, r.payment_deadline, r.status,
         (l.status = 'DELETED'::public.record_status)
  into v_renter_id, v_owner_id, v_listing_id, v_listing_title, v_start_at, v_end_at, v_payment_deadline, v_status,
       v_listing_deleted
  ...
  if v_status = 'PENDING_LATE'::public.reservation_status
     and v_uid = v_renter_id
     and v_uid <> v_owner_id then
    raise exception 'Late requests can only be cancelled by the listing owner';
  end if;
