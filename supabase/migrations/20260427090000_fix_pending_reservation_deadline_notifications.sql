...
set confirm_by_at = case
  when l.advance_notice_hours is null then null
  else r.start_at - make_interval(hours => greatest(l.advance_notice_hours, 0))
end
...
begin
  ...
  select
    ...
    case
      when l.advance_notice_hours is null then null
      else p_start_at - make_interval(hours => greatest(l.advance_notice_hours, 0))
    end
  into v_listing_owner_id, v_listing_status, v_listing_title, v_advance_notice_hours, v_confirm_by_at
  from public.listings l
  ...
  if v_confirm_by_at is not null and now() > v_confirm_by_at then
...
begin
  for v_cancelled in
    with updated as (
      ...
      where r.status = 'PENDING'::public.reservation_status
        and (
          r.start_at <= now()
          or (r.confirm_by_at is not null and r.confirm_by_at <= now())
        )
  ...
  loop
    ...
    perform public.notify_auto_cancelled_pending_reservation(
      ...
      case
        when v_cancelled.confirm_by_at is not null and v_cancelled.confirm_by_at <= now() then 'advance_notice_window_passed'
...
begin
  ...
  where n.type = 'pending_reservation_review_reminder'
    ...
    and not exists (
      ...
      where r.id::text = n.entity_id
        and r.status = 'PENDING'::public.reservation_status
        and least(r.start_at, coalesce(r.confirm_by_at, r.start_at)) > now()
    );
  ...
  for v_prompt in
    select
      ...
      r.renter_id,
      r.confirm_by_at,
      least(r.start_at, coalesce(r.confirm_by_at, r.start_at)) as review_deadline
    from public.reservations r
    join public.listings l on l.id = r.listing_id
    where r.status = 'PENDING'::public.reservation_status
      and l.owner_id is not null
      and least(r.start_at, coalesce(r.confirm_by_at, r.start_at)) > now()
      and now() >= least(r.start_at, coalesce(r.confirm_by_at, r.start_at)) - interval '30 minutes'
      and not exists (
        select 1
