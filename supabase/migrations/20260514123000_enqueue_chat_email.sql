...
create table if not exists public.chat_email_pending (
  ...
  primary key (chat_id, recipient_user_id)
);

create table if not exists public.chat_email_cooldowns (
  recipient_user_id uuid primary key references public."user"(id) on delete cascade,
...
begin
  for v_job_id in
    select jobid
    from cron.job
    where jobname = 'enqueue-chat-email'
  loop
  ...
  perform cron.schedule(
    ...
    '* * * * *',
    $job$
      select public.enqueue_chat_email();
    $job$
  );
end;
$$;
